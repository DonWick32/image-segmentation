...
class SAM2Train(SAM2Base):
    ...
    def forward_tracking(
        ...
        for stage_id in processing_order:
            ...
            # print("backbone_out['point_inputs_per_frame']", backbone_out["point_inputs_per_frame"])
            # print("backbone_out['mask_inputs_per_frame']", [(k, v.shape) for k, v in backbone_out["mask_inputs_per_frame"].items()])
            # print("backbone_out['gt_masks_per_frame']", [(k, v.shape) for k, v in backbone_out["gt_masks_per_frame"].items()])
            # print("backbone_out['frames_to_add_correction_pt']", frames_to_add_correction_pt)
            # print("outputdict",[(k, v.keys()) for k, v in output_dict['cond_frame_outputs'].items()])
            # # print(output_dict)
            # if 0 in output_dict['cond_frame_outputs']:
            #     print("OUTPUT DICT IS NOT EMPTY")
            #     for k,v in output_dict['cond_frame_outputs'][0].items():
            ...
            current_out = self.train_track_step(
                frame_idx=stage_id,
    ...
    def train_track_step(
        ...
        # print("before memory Cuda memory allocated", torch.cuda.memory_allocated())
        ...
        # print("after memory Cuda memory allocated", torch.cuda.memory_allocated())

        # current_out["multistep_pred_masks"] = current_out["multistep_pred_masks"].cpu()
        # current_out["multistep_pred_masks_high_res"] = current_out["multistep_pred_masks_high_res"].cpu()
        # current_out["multistep_pred_multimasks"] = [i.cpu() for i in current_out["multistep_pred_multimasks"]]
        # current_out["multistep_point_inputs"] = [point_inputs]

        # del low_res_masks, high_res_masks, low_res_multimasks, high_res_multimasks, ious, obj_ptr, object_score_logits
        # gc.collect()
        # torch.cuda.empty_cache()
        
        
        # print("after clean Cuda memory allocated", torch.cuda.memory_allocated())
    ...
    def add_new_points_or_box(
        ...
        # using any memory from other frames, like in SAM2 Otherwise (if it has been tracked),
